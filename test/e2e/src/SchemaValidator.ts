  ...
  private loadTypeScriptInstance(type: 'setup' | 'interaction', filename: string): TestSetupData | TestInteractionData {
    ...
    try {
      ...
      delete require.cache[modulePath];
        ...
        const camelCaseFilename = filename.charAt(0).toLowerCase() + filename.slice(1).replace(/-([a-z])/g, (g) => g[1].toUpperCase());
        const pascalCaseFilename = filename.charAt(0).toUpperCase() + filename.slice(1).replace(/-([a-z])/g, (g) => g[1].toUpperCase());
        const data = module[filename] || module[camelCaseFilename] || module[pascalCaseFilename] || module.default || module[`${filename}Setup`] || module[`${filename}Data`] || module[`${filename}Interaction`];
