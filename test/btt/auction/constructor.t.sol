...
contract AuctionConstructorTest is BttBase {
    ...
    function test_WhenTypeUint256MaxDivTotalSupplyGTUniV4MaxTick(AuctionFuzzConstructorParams memory _params) external
    setupAuctionConstructorParams(_params) {
    ...
    function test_WhenTypeUint256MaxDivTotalSupplyLTUniV4MaxTick(AuctionFuzzConstructorParams memory _params) external
    setupAuctionConstructorParams(_params) {
    ...
    function test_WhenClaimBlockGEEndBlock(AuctionFuzzConstructorParams memory _params) external
    setupAuctionConstructorParams(_params) {
        // it writes CLAIM_BLOCK
        // it writes VALIDATION_HOOK
        // it writes BID_MAX_PRICE

        _params.parameters.claimBlock = uint64(bound(_params.parameters.claimBlock, _params.parameters.endBlock, type(uint64).max));

        uint256 expectedBidMaxPrice = FixedPointMathLib.min(type(uint256).max / _params.totalSupply, ConstantsLib.MAX_BID_PRICE);
