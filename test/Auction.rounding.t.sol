...
contract ThanosSnap is AuctionBaseTest {
    ...
    /**
     * @notice Demonstrates how large ticks and weird bids make the auction do a thanos snap
     *
     * MECHANISM:
     * - Period 1: 1000 tokens bid at tick20, clears properly
     * - Period 2: 710 tokens bid at tick30 creates oversubscription
     *   - Exact clearing price falls between tick20 ($20) and tick30 ($30)
     *   - Snaps UP to tick30
     *   - Demand is not cleared, so accumulation gets messed up.
     *   - auction cannot be swept
     */
    function _test_largeTickSpacing_causesBurnAuction(bool _withSecondBid, bool _sweepEarly) public {
        ...
        bidId1 = mockAuction.submitBid{value: currency1}(tick20, true, currency1, bob, tick10, bytes(''));
        {
        ...
        emit log_named_decimal_uint('Cp1, cleared', ValueX7X7.unwrap(cp1.totalClearedX7X7) / 1e14, 18);
        ...
        if (_withSecondBid) {
            ...
            bidId2 = mockAuction.submitBid{value: currency2}(tick30, true, currency2, alice, tick20, bytes(''));
            {
            ...
            emit log_named_decimal_uint('Cp2, cleared', ValueX7X7.unwrap(cp2.totalClearedX7X7) / 1e14, 18);
...

        {
            ...
            emit log_named_decimal_uint('Cp3, cleared', ValueX7X7.unwrap(cp3.totalClearedX7X7) / 1e14, 18);
        ...
        uint256 cleared = ValueX7X7.unwrap(finalCheckpoint.totalClearedX7X7) / 1e14;

        emit log('==================== FINAL CHECKPOINT ====================');
        emit log_named_decimal_uint('Cleared', cleared, 18);
    ...
    function exitAndClaim(bool _sweepEarly) public {
...

        {
...

            {
                ...
                mockAuction.exitPartiallyFilledBid(bidId1, 1, 4);
            ...
            emit log_named_decimal_uint('Total cleared (reported)', cleared, 18);
            uint256 discrepancy = cleared - sumOfIndividualFills;
            emit log_named_decimal_uint('DISCREPANCY', discrepancy, 18);
    ...
    function _setupLargeTickAuction() internal {
        ...
        testParams.graduationThresholdMps = uint24((uint256(MPSLib.MPS) * 80) / 100);
