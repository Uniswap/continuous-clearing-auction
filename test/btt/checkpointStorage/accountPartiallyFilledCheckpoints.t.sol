...
contract AccountPartiallyFilledCheckpointsTest is BttBase {
    ...
    function test_WhenDemandEQ0(Bid memory _bid, ValueX7X7 _cumulativeCurrencyRaisedAtClearingPriceX7X7) external {
        // it returns (0, 0)

        (uint256 tokensFilled, uint256 currencySpent) = mockCheckpointStorage.accountPartiallyFilledCheckpoints(
            _bid, ValueX7.wrap(0), _cumulativeCurrencyRaisedAtClearingPriceX7X7
        );
    ...
    function test_WhenDemandGT0(Bid memory _bid, uint256 _tickDemand, uint256 _cumulativeCurrencyRaisedAtClearingPrice)
        external
    {
        ...
        _bid.amount = bound(_bid.amount, 0, type(uint128).max / 1e7);
        _tickDemand = bound(_tickDemand, 1, type(uint256).max / ConstantsLib.MPS);
        ...
        (uint256 tokensFilled, uint256 currencySpent) = mockCheckpointStorage.accountPartiallyFilledCheckpoints(
            _bid, ValueX7.wrap(_tickDemand), ValueX7X7.wrap(_cumulativeCurrencyRaisedAtClearingPrice)
        );

        uint256 left = ConstantsLib.MPS - _bid.startCumulativeMps;

        uint256 scaledCurrencySpent = FixedPointMathLib.fullMulDivUp(
            _bid.amount * 1e7, _cumulativeCurrencyRaisedAtClearingPrice, _tickDemand * left
        );
        ...
        uint256 directCurrencySpent =
            FixedPointMathLib.fullMulDivUp(_bid.amount, _cumulativeCurrencyRaisedAtClearingPrice, _tickDemand * left);
        ...
        uint256 rawCurrencySpent = (_bid.amount * _cumulativeCurrencyRaisedAtClearingPrice) / (_tickDemand * left);
