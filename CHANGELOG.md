...
Major release with breaking changes to the auction constructor, factory constructor, sweep authorization, the factory's distribution interface, and a number of interface renames. Adds supply rollover settlement math, a protocol fee controller, and new lens contracts for offchain reads. Integrators must redeploy and update integrations.
...
- `sweepCurrency()` no longer reverts for auctions that did not graduate; it now performs a no-op sweep (sweeping zero currency) rather than reverting `NotGraduated`. ([#343](https://github.com/Uniswap/continuous-clearing-auction/pull/343))
- **Breaking:** Standardized the factory's distribution interface to the `liquidity-launcher` submodule's `IDistributorFactory`. `initializeDistribution()` is renamed to `create()` and `getAuctionAddress()` is renamed to `getAddress()` (now returning `IDistributor`); the local `IDistributionStrategy` and `IDistributionContract` interfaces are removed. ([#356](https://github.com/Uniswap/continuous-clearing-auction/pull/356))
- Fixed bid submission to take the block checkpoint before invoking the validation hook. ([#346](https://github.com/Uniswap/continuous-clearing-auction/pull/346))
- Fixed `requiredDemandQ96AtNextActiveTick()` to return zero when there is no next active tick. ([#345](https://github.com/Uniswap/continuous-clearing-auction/pull/345))
- Prevented same-block `forceIterateOverTicks()` from changing bid admission: bid admission now uses the checkpoint clearing price, and force iteration reverts with the new `CheckpointAlreadyExistsForBlock` error when a checkpoint already exists for the current block. ([#355](https://github.com/Uniswap/continuous-clearing-auction/pull/355))
